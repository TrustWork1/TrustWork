...
const AppInfoList = () => {
  ...
  const { data, isLoading, refetch } = useQuery({
    ...
    queryFn: fetchAppInfoCmsList,
    retry: false
  })
  const cmsData: IHomeCmsModel['AppInfoList'] | undefined = useMemo(() => {
  ...
  const {
    ...
    clearErrors,
    formState: { errors }
  } = useForm<AppInfoValidationSchemaType>({
    defaultValues: initialDatas,
    resolver: yupResolver(appInfoValidationSchema),
    mode: 'onChange'
  })
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: updateAppInfoCmsList,
    onSuccess: res => {
  ...
  const onSubmit = (data: AppInfoValidationSchemaType) => {
