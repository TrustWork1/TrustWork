...
import {
  ...
  deleteHowItWorksStepsCmsList,
  fetchAppFeaturesCmsList,
  fetchHowItWorksStepsCmsList
} from 'src/services/functions/home-cms.api'
import AddEditDrawer from 'src/views/apps/landing-page-cms/home/how-it-works/list/AddEditDrawer'

import ConfirmDialog from 'src/@core/components/dialog'
import { globalSuccess } from 'src/lib/functions/_helpers.lib'
import TableHeaderAddFeatures from 'src/views/apps/landing-page-cms/home/how-it-works/list/TableHeader'
...
const RowOptions = ({ id, toggleMode }: { id: number | string; toggleMode: () => void }) => {
  ...
  const deleteMutation = useMutation({
    ...
    mutationFn: deleteHowItWorksStepsCmsList,
    onSuccess: response => {
      globalSuccess(response?.message)
      queryClient.invalidateQueries({ queryKey: [listOfUniqueKeys.home.howItworks.list] })
    }
  })
...
const AddStepsList = () => {
  ...
  const { data } = useQuery({ queryKey: [listOfUniqueKeys.home.howItworks.list], queryFn: fetchHowItWorksStepsCmsList })
  ...
  console.log(cmsData, 'Steps data')

  const columns: GridColDef[] = [
    {
      flex: 0.1,
      field: 'icon',
      headerName: 'Step Icon',
      sortable: false,
  ...
  return (
    <Grid container spacing={6.5}>
      <Grid item xs={12}>
        ...
        <Card>
          <CardHeader title='List of Steps' className='secondaryDesign' />
