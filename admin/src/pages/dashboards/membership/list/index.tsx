...
import { fetchMembership, updateMembershipStatus, deleteMembership } from 'src/services/functions/membership.api'
...
const ProviderList = () => {
  ...
  const { data, isLoading: fetchMembershipLoading } = useQuery({
    queryKey: ['fetchMembership', debouncedFilterValue, paginationModel],
    queryFn: () =>
      fetchMembership(debouncedFilterValue, { page: paginationModel.page + 1, pageSize: paginationModel.pageSize })
  })
  ...
  const updateStatusMutation = useMutation({
    mutationKey: ['updateMembershipStatus'],
  ...
  const columns: GridColDef[] = [
    ...
    //   renderCell: ({ row }: CellType) => (
    //     <Typography noWrap sx={{ color: 'text.secondary' }}>
    //       {row.plan_duration ? row.plan_duration : '-'}
    //     </Typography>
    //   )
    // },
    {
  ...
  return (
    <Grid container spacing={6.5}>
      <Grid item xs={12}>
        <Card>
          <CardHeader title='Manage Membership' className='secondaryDesign' />
          ...
          <DataGrid
            ...
            rows={providers}
            columns={columns}
            loading={fetchMembershipLoading}
