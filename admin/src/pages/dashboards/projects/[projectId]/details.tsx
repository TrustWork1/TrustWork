...
import assets from 'src/json/assets'
import AcceptedIcon from 'src/ui/Icons/AcceptedIcon'
import { TEachTransaction } from '@/type/apps/transaction.type'
import { fetchTransactionPerProject } from 'src/services/functions/transaction.api'
...
const ProviderBiddingDetails = () => {
  ...
  const { data: projectTransactionList, isLoading: projectTransactionLoading } = useQuery({
    queryKey: ['fetchTransactionPerProject', paginationModel, projectID],
    enabled: !!projectID,
    queryFn: () => fetchTransactionPerProject(projectID, '', { page: 1, pageSize: 100 })
  })
  const projectTransaction = useMemo(() => {
    return (
      projectTransactionList?.data?.map((item, index) => ({
        ...item,
        unique_id: `${item.escrow_id}_${index}`
      })) ?? []
    )
  }, [projectTransactionList?.data])
  ...
  const columns: GridColDef[] = [
    ...
    // },
    {
      ...
      renderCell: ({ row }: CellType) => (
        ...
        >
          {row?.status?.toLowerCase() === 'accepted' ? <AcceptedIcon /> : null}
        </figure>
      )
    },
    {
    ...
    },
    {
      ...
      headerName: 'Timeline',
      renderCell: ({ row }: CellType) => (
        <Typography noWrap sx={{ color: 'text.secondary' }}>{`${row?.time_line || 'NA'} ${
          row?.time_line_hour || 'NA'
        }`}</Typography>
      )
    },
    {
  ...
  const transactionColumns: GridColDef[] = [
    ...
    },
    {
      ...
      disableColumnMenu: true,
      renderCell: ({ row }: TransactionCellType) => {
        console.log(row, 'rowsdfsdfsdfsd')
      ...
      <Grid item xs={12}>
          ...
          <Box sx={{ p: theme => theme.spacing(0, 6, 6), mt: 8 }}>
            <Grid container>
              ...
              <Grid item md={6} sx={{ px: 2, mb: 3 }}>
                ...
                <Typography variant='body2'>{`${projectDetails?.project_timeline || 'NA'} ${
                  projectDetails?.project_hrs_week || 'NA'
                }`}</Typography>
              </Grid>
              <Grid item md={6} sx={{ px: 2, mb: 3 }}>
                <Typography>
        ...
        <Card sx={{ mt: 5 }}>
          ...
          <DataGrid
            ...
            rows={projectTransaction}
            columns={transactionColumns}
            loading={projectTransactionLoading}
