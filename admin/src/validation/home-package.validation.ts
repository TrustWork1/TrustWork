...
export const packageCreateValidationSchema = () => {
  return yup.object().shape({
    plan_name: yup.string().required('Plan name is required').matches(/^\S+$/, 'No blank spaces allowed in plan name'),
    ...
    features: yup
      ...
      .of(
        yup.object().shape({
          features: yup.string().required('Feature is required').matches(/^\S+$/, 'No blank spaces allowed in feature'),
