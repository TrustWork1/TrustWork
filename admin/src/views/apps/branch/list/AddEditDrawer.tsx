...
// ** API Imports
import { fetchBranchById, storeBranch, updateBranch } from 'src/services/functions/branch.api'
...
const SidebarAddBranch = (props: SidebarAddBranchType) => {
  ...
  useEffect(() => {
    if (data?.data && mode === 'edit') {
      setValue('branch_id', data.data._id)
  ...
  const storeMutator = useMutation({
    mutationKey: ['storeBranch'],
    mutationFn: storeBranch,
    onSuccess: response => {
      globalSuccess(response?.message)
      queryClient.invalidateQueries({ queryKey: ['fetchBranches'] })
    }
  ...
  return (
    ...
    >
      <Header>
        <Typography variant='h5'>{props.mode === 'add' ? 'Add Branch' : 'Edit Branch'}</Typography>
        <IconButton
          size='small'
          onClick={handleClose}
          sx={{
