...
const SidebarAddEdit = (props: SidebarAddEditType) => {
  ...
  const updateMutator = useMutation({
    mutationKey: [listOfUniqueKeys.home.appFeatures.update, id],
    mutationFn: (formData: FormData) => updateAppFeaturesCmsList(id as string, formData),
    onSuccess: response => {
  ...
  const storeMutator = useMutation({
    mutationKey: [[listOfUniqueKeys.home.appFeatures.save]],
    mutationFn: (formData: FormData) => saveAppFeaturesCmsList(formData),
    onSuccess: response => {
      globalSuccess(response?.message)
      queryClient.invalidateQueries({ queryKey: [listOfUniqueKeys.home.appFeatures.list] })
    }
  ...
  return (
    ...
    >
      <Header>
        <Typography variant='h5'>{props.mode === 'add' ? 'Add ' : 'Edit '}Feature</Typography>
        <IconButton
          size='small'
          onClick={handleClose}
          sx={{
      ...
      <Box sx={{ p: theme => theme.spacing(0, 6, 6) }}>
        ...
        >
          <Grid container>
            ...
            {/* Plan benefits */}
            <Grid item md={12} sx={{ px: 3, mb: 3 }}>
              <label className='project_title'>Feature Description*</label>
