...
const SidebarAddEdit = (props: SidebarAddEditType) => {
  ...
  const updateMutator = useMutation({
    mutationKey: [listOfUniqueKeys.home.howItworks.update, id],
    mutationFn: (formData: FormData) => updateHowItWorksStepsCmsList(id as string, formData),
    onSuccess: response => {
  ...
  const storeMutator = useMutation({
    mutationKey: [[listOfUniqueKeys.home.howItworks.save]],
    mutationFn: (formData: FormData) => saveHowItWorksStepsCmsList(formData),
    onSuccess: response => {
      globalSuccess(response?.message)
      queryClient.invalidateQueries({ queryKey: [listOfUniqueKeys.home.howItworks.list] })
    }
  ...
  return (
    ...
    >
      <Header>
        <Typography variant='h5'>{props.mode === 'add' ? 'Add ' : 'Edit '}Step</Typography>
        <IconButton
          size='small'
          onClick={handleClose}
          sx={{
      ...
      <Box sx={{ p: theme => theme.spacing(0, 6, 6) }}>
        ...
        >
          <Grid container>
            ...
            {/* Plan benefits */}
            <Grid item md={12} sx={{ px: 3, mb: 3 }}>
              <label className='project_title'>Step Description*</label>
